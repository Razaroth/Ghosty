...
struct Ghost
{
    QRectF frac; // 0..1 relative rect
    int alpha;
};

// Mirrors src/icon.cpp drawGhost(); r is in PIXELS.
void ghost(QPainter &p, const QRectF &r, int fillAlpha)
{
    ...
    QPainterPath body;
    body.moveTo(0.28 * w, 0.80 * h);
    body.lineTo(0.28 * w, 0.30 * h);
    body.cubicTo(0.28 * w, 0.05 * h, 0.72 * w, 0.05 * h, 0.72 * w, 0.30 * h);
    body.lineTo(0.72 * w, 0.80 * h);
    body.quadTo(0.66 * w, 0.94 * h, 0.56 * w, 0.80 * h);
    body.quadTo(0.50 * w, 0.94 * h, 0.44 * w, 0.80 * h);
    body.quadTo(0.38 * w, 0.94 * h, 0.28 * w, 0.80 * h);
    body.closeSubpath();
...
void drawAll(QPainter &p, const QList<Ghost> &ghosts, int s)
{
    for (const Ghost &g : ghosts) {
        ghost(p, QRectF(g.frac.x() * s, g.frac.y() * s, g.frac.width() * s, g.frac.height() * s), g.alpha);
    }
}

// SVG shapes are expressed in 0..1 fractional units (viewBox 0 0 1 1).
                 ...
                 QString &arm, QString &blush, QString &eyes, QString &mouth)
{
    ...
    path = QString::fromLatin1("M %1 %2 L %3 %4 C %5 %6 %7 %8 %9 %10 L %11 %12 Q %13 %14 %15 %16 Q %17 %18 %19 %20 Q %21 %22 %23 %24 Z")
        .arg(n("%.4f", ox + 0.28 * w), n("%.4f", oy + 0.80 * h),
             n("%.4f", ox + 0.28 * w), n("%.4f", oy + 0.30 * h),
             n("%.4f", ox + 0.28 * w), n("%.4f", oy + 0.05 * h),
             n("%.4f", ox + 0.72 * w), n("%.4f", oy + 0.05 * h),
             n("%.4f", ox + 0.72 * w), n("%.4f", oy + 0.30 * h),
             n("%.4f", ox + 0.72 * w), n("%.4f", oy + 0.80 * h),
             n("%.4f", ox + 0.66 * w), n("%.4f", oy + 0.94 * h),
             n("%.4f", ox + 0.56 * w), n("%.4f", oy + 0.80 * h),
             n("%.4f", ox + 0.50 * w), n("%.4f", oy + 0.94 * h),
             n("%.4f", ox + 0.44 * w), n("%.4f", oy + 0.80 * h),
             n("%.4f", ox + 0.38 * w), n("%.4f", oy + 0.94 * h),
             n("%.4f", ox + 0.28 * w), n("%.4f", oy + 0.80 * h));
    arm = QString::fromLatin1("<ellipse cx=\"%1\" cy=\"%2\" rx=\"%3\" ry=\"%4\"/>\n  "
                              "<ellipse cx=\"%5\" cy=\"%6\" rx=\"%7\" ry=\"%8\"/>")
        .arg(n("%.4f", ox + 0.195 * w), n("%.4f", oy + 0.59 * h), n("%.4f", 0.085 * w), n("%.4f", 0.17 * h),
             n("%.4f", ox + 0.805 * w), n("%.4f", oy + 0.59 * h), n("%.4f", 0.085 * w), n("%.4f", 0.17 * h));
    blush = QString::fromLatin1("<ellipse cx=\"%1\" cy=\"%2\" rx=\"%3\" ry=\"%4\"/>\n  "
                                "<ellipse cx=\"%5\" cy=\"%6\" rx=\"%7\" ry=\"%8\"/>")
        .arg(n("%.4f", ox + 0.36 * w), n("%.4f", oy + 0.515 * h), n("%.4f", 0.06 * w), n("%.4f", 0.035 * h),
             n("%.4f", ox + 0.64 * w), n("%.4f", oy + 0.515 * h), n("%.4f", 0.06 * w), n("%.4f", 0.035 * h));
    eyes = QString::fromLatin1("<ellipse cx=\"%1\" cy=\"%2\" rx=\"%3\" ry=\"%4\"/>\n  "
                               "<ellipse cx=\"%5\" cy=\"%6\" rx=\"%7\" ry=\"%8\"/>")
        .arg(n("%.4f", ox + 0.425 * w), n("%.4f", oy + 0.365 * h), n("%.4f", 0.045 * w), n("%.4f", 0.065 * h),
             n("%.4f", ox + 0.575 * w), n("%.4f", oy + 0.365 * h), n("%.4f", 0.045 * w), n("%.4f", 0.065 * h));
    mouth = QString::fromLatin1("<ellipse cx=\"%1\" cy=\"%2\" rx=\"%3\" ry=\"%4\"/>")
        .arg(n("%.4f", ox + 0.50 * w), n("%.4f", oy + 0.59 * h), n("%.4f", 0.035 * w), n("%.4f", 0.04 * h));
}
...
QString svgGhost(const QRectF &r, int alphaPct)
{
    ...
    return QString::fromLatin1(
        "<g opacity=\"%1\">\n"
        "  <path d=\"%2\" fill=\"#ECF1F9\" stroke=\"#96A0B4\" stroke-width=\"0.045\" stroke-linejoin=\"round\"/>\n"
        "  <g fill=\"#ECF1F9\" stroke=\"#96A0B4\" stroke-width=\"0.045\" stroke-linejoin=\"round\">\n  %3\n  </g>\n"
        "  <g fill=\"#F0A8B4\">\n  %4\n  </g>\n"
        "  <g fill=\"#343A4A\">\n  %5\n  %6\n  </g>\n"
        "</g>")
        .arg(alphaPct)
...
int main(int argc, char **argv)
{
    ...
    // Same layout as src/icon.cpp::paint().
